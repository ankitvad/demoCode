...
trgLOC = {"LT": "/home/avadehra/scribendi/dataAudio/parsed/LIB_TED/dev_trg.txt", "RED": "/home/avadehra/scribendi/dataAudio/parsed/RED/dev_trg.txt"}
srcLOC = {"LT": "/home/avadehra/scribendi/dataAudio/parsed/LIB_TED/dev_src0.txt", "RED": "/home/avadehra/scribendi/dataAudio/parsed/RED/dev_src.txt"}


allC = glob.glob("EP5_FT/*/*/*/gen_output_beam5.txt") + glob.glob("/home/avadehra/scribendi/audio-HF/models-*/*/*/gen_output_beam5.txt")


#Write out the result in the file:
writeOut = open("all_results_WER.txt", "w")

for f in tqdm(allC):
	tmpName = f.strip("/").split("/")
	assert(len(tmpName) in [5,8]), "File path structure not as expected."
	if len(tmpName) == 5:
		nm = "-".join(tmpName[1:3])
		if "LT" in nm:
			test_trg_LOC = trgLOC["LT"]
			test_src_LOC = srcLOC["LT"]
		elif "RED" in nm:
			test_trg_LOC = trgLOC["RED"]
			test_src_LOC = srcLOC["RED"]
		else:
			raise ValueError("Unknown dataset in file path.")
	elif len(tmpName) == 8:
		nm = "-".join(tmpName[4:6])
		if "LT" in nm:
			test_trg_LOC = trgLOC["LT"]
			...
			raise ValueError("Unknown dataset in file path.")
	#Read in the test trg and src files:
