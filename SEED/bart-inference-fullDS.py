...
LT_LOC = glob.glob("LT*FT/*/*/*/wer_scores.txt")
RED_LOC = glob.glob("RED*FT/*/*/*/wer_scores.txt")

LT_FILE = "/home/avadehra/scribendi/dataAudio/parsed/ASR_D_T/lib_ted_src0.txt"
RED_FILE = "/home/avadehra/scribendi/dataAudio/parsed/ASR_D_T/red_src.txt"

model_name = sys.argv[1]

assert(model_name in ["LT", "RED"]), "Please provide a valid model name: LT or RED."
...
indx_range = sys.argv[2].strip().split("-")
assert len(indx_range) == 2, "Please provide a valid range for the model checkpoints."
indx_range = [int(i) for i in indx_range]

#Load all the checkpoints in the model_loc folder after removing the wer_scores.txt part.
#Add the last checkpoint folder to the model_name variable for each location.
#example: 'RED_BB_FT/Base50/corrup_1x/400/wer_scores.txt'
all_locations = []
for m in model_loc:
	base_loc = m.replace("wer_scores.txt", "").rstrip("/")
	checkpoint_folders = glob.glob(base_loc + "/checkpoint-*/")
	last_step = -1
	last_checkpoint = ""
	for c in checkpoint_folders:
		step = int(c.rstrip("/").split("/")[-1].split("-")[-1])
		if step > last_step:
			last_step = step
			last_checkpoint = c
	all_locations.append(last_checkpoint)


all_locations = sorted(all_locations)
print("Total number of model checkpoints to process: ", len(all_locations))
...
for m in tqdm(all_locations):
	...
	op_dir = model_name.split("checkpoint-")[0] + "gen_output_beam4_wholeDS.txt"
