...
parser = argparse.ArgumentParser(description="BART synthetic-data Script")
...
parser.add_argument('--add_gt', action='store_true', help='Whether to add ground truth data to training')
parser.add_argument('--gt_src', type=str, default='', help='Path to ground truth source file (optional)')
parser.add_argument('--gt_trg', type=str, default='', help='Path to ground truth target file (optional)')
...
op_dir = args.op_dir.rstrip("/") + str("/") + str(args.seed) + "/"
...
if args.add_gt:
	assert args.gt_src != '' and args.gt_trg != '', "Ground truth source and target files must be provided when --add_gt is set."
	gt_ds = loadData(args.gt_src, args.gt_trg)
	train_ds = pd.concat([train_ds, gt_ds], ignore_index=True)
